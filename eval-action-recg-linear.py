...
def run_phase(phase, loader, model, optimizer, epoch, args, cfg, logger):
    ...
    batch_time = main_utils.AverageMeter('Time', ':6.3f', 100)
    data_time = main_utils.AverageMeter('Data', ':6.3f', 100)
    loss_meters = {ft: main_utils.AverageMeter('Loss', ':.4e', 0) for ft in feature_names}
    top1_meters = {ft: main_utils.AverageMeter('Acc@1', ':6.2f', 0) for ft in feature_names}
    top5_meters = {ft: main_utils.AverageMeter('Acc@5', ':6.2f', 0) for ft in feature_names}
    progress = {'timers': main_utils.ProgressMeter(len(loader), meters=[batch_time, data_time], phase=phase, epoch=epoch, logger=logger)}
    progress.update({ft: main_utils.ProgressMeter(len(loader), meters=[loss_meters[ft], top1_meters[ft], top5_meters[ft]], phase=phase, epoch=epoch, logger=logger) for ft in feature_names})
    ...
    if args.distributed:
        for ft in progress:
            main_utils.synchronize_meters(progress[ft], args.gpu)
